...
const Progress = ({ step }) => {
  return (
    <div class="flex flex-row-reverse justify-center items-center flex-grow-0 flex-shrink-0 w-[74px] h-8 relative gap-2.5">
      <svg
        width="74"
        height="32"
        viewBox="0 0 74 32"
      ...
      >
        ...
        <defs>
          ...
          >
            <stop
              offset="0.4"
              stop-color={step === 1 ? "#E2FFB2" : "#F3F4F1"}
            ></stop>
            <stop
              offset="0.55"
              stop-color={step === 1 ? "#F3F4F1" : "#E2FFB2"}
            ></stop>
          </linearGradient>
        </defs>
      </svg>
      <p
        class={`flex-grow w-8 text-xs text-center ${
